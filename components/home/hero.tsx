      ...
      <section className="relative min-h-[80vh] flex items-end pt-24 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 w-full">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.25, 0.46, 0.45, 0.94] }}>
            {post.section_slug && (
                <p className="text-[hsl(var(--accent))] text-xs font-sans font-semibold uppercase tracking-widest mb-4">{post.section_slug}</p>
            )}
            <Link href={`/post/${post.slug}`}>
              <h1 className="text-white text-4xl md:text-6xl font-bold leading-tight mb-6 hover:text-[hsl(42_20%_88%)] transition-colors"
                  style={{ fontFamily: '"Playfair Display", serif' }}>
                {post.title}
              </h1>
            </Link>
            {post.summary && (
                <p className="text-[hsl(42_20%_75%)] text-lg font-sans max-w-2xl mb-6 leading-relaxed">{post.summary}</p>
            )}
            <div className="flex items-center gap-4 text-sm font-sans text-[hsl(42_20%_60%)]">
