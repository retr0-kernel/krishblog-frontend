  ...
  return (
    <div className="relative group my-6">
      <div className="flex items-center justify-between bg-[hsl(20_14%_12%)] px-4 py-2 rounded-t-lg">
        ...
        <button
          onClick={copy}
          className="text-[hsl(42_20%_50%)] hover:text-white transition-colors"
...
function renderBlock(block: PostBlock) {
  switch (block.type) {
    ...
    case "code":
      return <CodeBlock key={block.id} content={block.content} attrs={block.attrs} />;
    case "image":
      return (
        <figure key={block.id} className="my-8">
          ...
          {typeof block.attrs?.caption === "string" && (
            <figcaption className="text-center text-xs font-sans text-[hsl(var(--muted-foreground))] mt-2">
              {block.attrs.caption}
            </figcaption>
          )}
        </figure>
    ...
    case "divider":
      return (
        <div key={block.id} className="my-10 flex items-center justify-center gap-4">
          {[0, 1, 2].map((i) => (
            <div key={i} className="w-1 h-1 rounded-full bg-[hsl(var(--accent))]" />
          ))}
        </div>
      );
    case "callout":
