...
const COLORS = ["hsl(14 80cle% 52%)", "hsl(220 80% 55%)", "hsl(142 71% 45%)", "hsl(260 60% 55%)", "hsl(40 80% 50%)"];
...
export default function AdminAnalyticsPage() {
    ...
    useEffect(() => {
        if (!token) return;
        setLoading(true);
        adminGetOverview(token, days).then(setStats).catch(() => {}).finally(() => setLoading(false));
    }, [token, days]);

    const metrics = [
        { label: "Page Views", value: stats?.total_page_views ?? 0, icon: Eye },
        { label: "Unique Visitors", value: stats?.unique_visitors ?? 0, icon: Users },
        { label: "Avg Scroll %", value: stats ? `${stats.avg_scroll_pct.toFixed(0)}%` : "0%", icon: TrendingUp },
        { label: "Avg Read Time", value: stats ? `${stats.avg_read_time_sec.toFixed(0)}s` : "0s", icon: Clock },
    ];
                        ...
                        {metrics.map((m, i) => (
                            <motion.div key={m.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                    <m.icon className="h-4 w-4 text-[hsl(var(--accent))]" />
                    ...
                    {stats?.daily_views && stats.daily_views.length > 0 && (
                        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                                <LineChart data={stats.daily_views} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="mt-1 bg-[hsl(var(--muted))] rounded-full h-1.5 overflow-hidden">
                                                    <div className="h-full bg-[hsl(var(--accent))] rounded-full"
                                                         style={{ width: `${(p.views / (stats.top_posts[0]?.views || 1)) * 100}%` }} />
                                                </div>
                                            </div>
                            ...
                            {(!stats?.device_breakdown || stats.device_breakdown.length === 0)
                                ...
                                : <>
                                    <ResponsiveContainer width="100%" height={140}>
                                        <PieChart>
                                            <Pie data={stats.device_breakdown} dataKey="count" nameKey="device" cx="50%" cy="50%" innerRadius={40} outerRadius={60}>
                                                {stats.device_breakdown.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                                            </Pie>
                                    ...
                                    <div className="space-y-1.5 mt-2">
                                        {stats.device_breakdown.map((d, i) => (
                                            <div key={d.device} className="flex items-center justify-between text-xs font-sans">
                                                ...
                                                <span className="text-[hsl(var(--muted-foreground))]">{d.pct.toFixed(0)}%</span>
                                            </div>
                                        ))}
                                    </div>
                                </>
                    ...
                    <div className="grid lg:grid-cols-2 gap-6">
                        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
                            ...
                            {(!stats?.top_referrers || stats.top_referrers.length === 0)
                                ...
                                : <div className="space-y-2">
                                    {stats.top_referrers.slice(0, 6).map((r) => (
                        ...
                        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
                            ...
                            {(!stats?.country_breakdown || stats.country_breakdown.length === 0)
                                ? <p className="text-sm font-sans text-[hsl(var(--muted-foreground))] text-center py-6">No data</p>
                                : <div className="space-y-2">
                                    {stats.country_breakdown.slice(0, 6).map((c, i) => (
                                        <div key={c.country} className="flex items-center gap-3 text-sm font-sans">
                                            ...
                                            <span className="ml-auto font-semibold">{c.pct.toFixed(0)}%</span>
                                        </div>
                                    ))}
                                </div>
                            }
